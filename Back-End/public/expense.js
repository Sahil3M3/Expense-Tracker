...
if(expenseId)
{
  ...
  axios.put(`http://52.65.52.207:80/expense/${expenseId}`,expense,{headers:{"Authorization":token}})
  .then(r=>{
...
{
  const token=localStorage.getItem("jwt")
  axios.post('http://52.65.52.207:80/expense',expense,{headers:{"Authorization":token}})
  .then(r=>{
    id=r.data.id;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  axios.get("http://52.65.52.207:80/premium/check", { headers: { "Authorization": token } })
    .then(r => {
...
function fetchData(page, limit) {
  ...
  axios.get(`http://52.65.52.207:80/expense?page=${page}&limit=${limit}`, { headers: { "Authorization": token } })
    .then(r => {
...
function fetchDownload() {
  const token = localStorage.getItem("jwt");
  axios.get('http://52.65.52.207:80/expense/getdownload', { headers: { "Authorization": token } })
    .then(r => {
...
function handleDelete(id)
{
  console.log(id);
  const token=localStorage.getItem("jwt")
axios.delete(`http://52.65.52.207:80/expense/${id}`,{headers:{"Authorization":token}})
.then(r=>{
  console.log(r);
})
.catch(e=>console.log(e))
...
axios.get('http://52.65.52.207:80/premium/premiummembership',{headers:{Authorization:token}})
.then(r=>{
...
console.log("order id "+orderid);
        var options = {
          ...
          "handler": function(response) {
            ...
            axios.post('http://52.65.52.207:80/premium/premiummembership', payment, { headers: { Authorization: token } })
            .then(res => {
...
function showLeaderboard(event)
{
  ...
  axios.get('http://52.65.52.207:80/expense/board',{headers:{"Authorization":token}})
  .then(r=>{
...
async function handleDownload(e) {
  ...
  try {
    ...
    const response = await  axios.get('http://52.65.52.207:80/expense/download', {
